...
export default function UserList() {
  ...
  const [filterType, setFilterType] = useState('month'); // 'month' o 'day'
  ...
  // Estados específicos para el paginador del modal
const [analyticsCurrentPage, setAnalyticsCurrentPage] = useState(0);
...
const fetchAnalyticTime = async (employeeId) => {
    try {
        ...
        const response = await fetch(
          `${API_URL}/links/analytic_time_combinado/${employeeId}/${formattedDate}?filterType=${filterType}`
        );
      console.log("La fecha devuelta es: " + formattedDate );
  ...
  const renderAnalyticsPagination = () => {
    const totalItems = analyticData.length;
    const totalPages = Math.ceil(totalItems / analyticsItemsPerPage);
    ...
    if (totalPages <= maxVisiblePages) {
      ...
      if (analyticsCurrentPage <= maxPagesBeforeCurrent) {
        startPage = 1;
        endPage = maxVisiblePages;
      } else if (analyticsCurrentPage + maxPagesAfterCurrent >= totalPages) {
        startPage = totalPages - maxVisiblePages + 1;
        endPage = totalPages;
      } else {
        startPage = analyticsCurrentPage - maxPagesBeforeCurrent;
        endPage = analyticsCurrentPage + maxPagesAfterCurrent;
      }
    }
    ...
    const paginate = (page) => {
      setAnalyticsCurrentPage(page - 1); // Restamos 1 porque trabajamos con índice 0
    };
  
    const handleItemsPerPageChange = (e) => {
      setAnalyticsItemsPerPage(Number(e.target.value));
      setAnalyticsCurrentPage(0);
    };
    ...
    return (
      <div className="flex items-center justify-between mt-4">
        <div className="flex items-center space-x-2">
          <span className="text-sm text-gray-700">
            Mostrando {(analyticsCurrentPage * analyticsItemsPerPage) + 1}-{Math.min(
        ...
        <div className="flex space-x-1">
          <button
            onClick={() => paginate(1)}
            disabled={analyticsCurrentPage === 0}
            className={`px-3 py-1 rounded-md ${analyticsCurrentPage === 0 ? 'bg-gray-200 text-gray-500 cursor-not-allowed' : 'bg-blue-50 text-blue-600 hover:bg-blue-100'}`}
          >
            «
          </button>
          
          <button
            onClick={() => paginate(analyticsCurrentPage)}
            disabled={analyticsCurrentPage === 0}
            className={`px-3 py-1 rounded-md ${analyticsCurrentPage === 0 ? 'bg-gray-200 text-gray-500 cursor-not-allowed' : 'bg-blue-50 text-blue-600 hover:bg-blue-100'}`}
          >
            ‹
          ...
          {pageNumbers.map(number => (
            <button
              key={number}
              onClick={() => paginate(number)}
              className={`px-3 py-1 rounded-md ${analyticsCurrentPage + 1 === number ? 'bg-blue-600 text-white' : 'bg-blue-50 text-blue-600 hover:bg-blue-100'}`}
            >
              {number}
            </button>
          ))}
          ...
          <button
            onClick={() => paginate(analyticsCurrentPage + 2)}
            disabled={analyticsCurrentPage + 1 === totalPages}
            className={`px-3 py-1 rounded-md ${analyticsCurrentPage + 1 === totalPages ? 'bg-gray-200 text-gray-500 cursor-not-allowed' : 'bg-blue-50 text-blue-600 hover:bg-blue-100'}`}
          >
            ›
          </button>
              ...
              <table className="w-full min-w-[800px]">
                <thead className="sticky top-0 bg-gray-100">
                  <tr>
                    ...
                    <th className="p-2 text-sm border-b">Tiempo</th>
  ...
  <div className="mt-6 bg-white p-4 rounded-lg shadow-md">
    <h3 className="text-xl font-bold mb-4 italic">
      {filterType === 'month' 
        ? `Todas las tareas del Mes ${selectedDate.substring(0, 7)}`
        : `Tareas del dia seleccionado ${selectedDate}`}
    </h3>
    ...
    {filterType === 'month' ? (
      <div className="text-center">
        <table className="w-full border-collapse">
          ...
          <tbody>
            <tr>   
              <td className="text-blue-600">{analyticData.Nombre}</td>
            </tr> 
            {Array.isArray(analyticData) ? (
              <>
                {analyticData
                  .slice(
                  ...
                  .map((entry, index) => (
                    <tr key={index}>
                      ...
                      <td className="p-2 border text-left">{entry.tarea}</td>
                      <td className="p-2 border">{entry.horas.toFixed(2)}h</td>
                      <td className="p-2 border">{entry.proyecto}</td>
                    </tr>
                  ))}
                <tr className="font-bold bg-gray-100">
                  <td className="p-2 border" colSpan="2">Total horas:</td>
                  <td className="p-2 border">
                    {analyticData.reduce((total, entry) => total + Number(entry.horas.toFixed(2)), 0)}h
                  </td>
              ...
              <tr>
                <td colSpan="4" className="p-2 border text-center">No hay datos</td>
              </tr>
            )}
          </tbody>
        </table>
          ...
          {Array.isArray(analyticData) ? (
            analyticData
              ...
              .map((entry, index) => (
                <tr key={index}>
                  <td className="p-2 border">{entry.date}</td>
                  <td className="p-2 border">{entry.tarea}</td>
                  <td className="p-2 border">{entry.proyecto}</td>
                  <td className="p-2 border">{entry.horas.toFixed(2)}h</td>
                </tr>
              ))
    ...
    {Array.isArray(analyticData) && analyticData.length > analyticsItemsPerPage && renderAnalyticsPagination()}
  </div>
)}
       ...
       {/* Modal de Analytics con paginador */}
