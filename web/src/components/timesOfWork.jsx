...
      try {
        const response = await fetch(`${API_URL}/links/analytic_time_time/${employee.employee_id}/${formatLocalDate(employee.date)}`
        );
        
...
          Detalles de Tiempo en el Mes: {employee?.name} - {format(employee?.date, '/yyyy/MM')}
        </h2>
        <button
          onClick={onClose}
...
    const dateString = formatMonthLocalDate(date); // Usar nuevo formateador
