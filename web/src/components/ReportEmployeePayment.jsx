...
const [isUpdating, setIsUpdating] = useState(false);
  ...
  const handleValidationToggle = (paymentId) => {
    setAllPayments(prevPayments => 
      prevPayments.map(payment => 
        payment.id === paymentId 
          ? { ...payment, validado: payment.validado ? 0 : 1 } 
          : payment
      )
    );
  };
  ...
  const handleUpdateValidations = async () => {
    try {
      setIsUpdating(true);
      const updates = allPayments.map(payment => ({
        id: payment.id,
        validado: payment.validado
      }));

      console.log("Updates que paso a la ruta: " + updates);

      const response = await fetch(`${API_URL}/links/updateValidations`, {
        method: 'PUT',
        headers: {
      ...
      if (!response.ok) throw new Error('Error al actualizar validaciones');

      // Recargar datos actualizados
      const dateObj = new Date(selectedDate);
      const year = dateObj.getFullYear();
      const month = String(dateObj.getMonth() + 1).padStart(2, '0');
      const res = await fetch(`${API_URL}/links/getAllPayments/${year}/${month}`);
      const data = await res.json();
      setAllPayments(data);
      
    } catch (err) {
      console.error('Error actualizando validaciones:', err);
      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden">
        <div className="p-4 border-b border-gray-200 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="flex flex-col gap-4 sm:flex-row items-start sm:items-center">
            ...
            >
              {isUpdating ? 'Actualizando...' : 'Actualizar Validaciones'}
            </button>
                      ...
                      <tr key={payment.id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                          <input
                            type="checkbox"
                            checked={validado}
                            onChange={() => handleValidationToggle(payment.id)}
