...
router.post('/runscript_employee', async (req, res) => {
  const scriptPath = path.join(__dirname, '/getDbEmployee.py');
...
router.put('/updateValidations', async (req, res) => {
  ...
  try {
    ...
    if (!Array.isArray(req.body) || req.body.length === 0) {
      return res.status(400).json({
        success: false,
        message: 'Se requiere un array de objetos con id y validado'
      });
    }
    ...
    try {
     ...
     // await beginTransaction();
      ...
      for (const { id, validado } of req.body) {
        console.log("PAso por: " + id);
        console.log("Validado?: " + validado);
        await pool.query(
          'UPDATE employee_payments SET validado = ? WHERE id_employee = ?',
          [validado, id]
        );
      }
      ...
      res.json({
        success: true,
        message: `${req.body.length} registros actualizados`,
        updatedCount: req.body.length
      });
      ...
      await rollback();
      throw error; // Re-lanzar el error para el catch externo
    } finally {
      // 8. Liberar conexión siempre
      connection.release();
    }

  } catch (error) {
    console.error('Error en updateValidations:', error);
    res.status(500).json({
      success: false,
      message: 'Error al actualizar validaciones',
...
router.get('/analytic_time_time/:employee_id/:date', async (req, res) => {
  try {
    const { employee_id, date } = req.params;
    ...
    SELECT 
      ...
      WHERE A.employee_id = ?
        AND substring(A.date,6,2) = substring(?,6,2)
      ORDER BY A.date, A.name ASC  
    `, [employee_id, date]);
...
router.get('/analytic_time/:employee_id/:date', async (req, res) => {
  try {
    ...
    console.log("Entro a filtrar por mes en Analytic... :" + fecha);
    const results = await pool.query(
      `SELECT 
        E.id,
        E.name AS empleado,
...
router.post('/update-employee-payments', async (req, res) => {
  ...
  console.log("Recibiendo en Node Fecha: " + selectedDate);
    ...
    const newPayments = await pool.query(
      `INSERT INTO employee_payments
          ...
          WHERE 
              MONTH(eps.date) = ? 
              AND YEAR(eps.date) = ?     
          GROUP BY eps.employee_id, e.id, substring(eps.date,6,2)
          RETURNING *`,
